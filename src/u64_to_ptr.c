...
u64_to_ptr * u64_to_ptr_create(const char * optional_name){
  ...
  static const char * const column_types[] = {"u64", "void_ptr"};
  ...
  icy_table_init((icy_table * )instance, optional_name, 2, (unsigned int[]){sizeof(u64), sizeof(void_ptr)}, (char *[]){(char *)"key", (char *)"value"});
  
  return instance;
}

void u64_to_ptr_insert(u64_to_ptr * table, u64 * key, void_ptr * value, size_t count){
  void * array[] = {(void* )key, (void* )value};
  icy_table_inserts((icy_table *) table, array, count);
}

void u64_to_ptr_set(u64_to_ptr * table, u64 key, void_ptr value){
  void * array[] = {(void* )&key, (void* )&value};
...
bool u64_to_ptr_try_get(u64_to_ptr * table, u64 * key, void_ptr * value){
  void * array[] = {(void* )key, (void* )value};
  ...
  unsigned int sizes[] = {sizeof(u64), sizeof(void_ptr)};
  for(int i = 1; i < 2; i++){
    if(array[i] != NULL)
