...
u32_table * u32_table_create(const char * optional_name){
  static const char * const column_names[] = {(char *)"key"};
  static const char * const column_types[] = {"u32"};
  u32_table * instance = calloc(sizeof(u32_table), 1);
  instance->column_names = (char **)column_names;
  instance->column_types = (char **)column_types;
  
  icy_table_init((icy_table * )instance, optional_name, 1, (unsigned int[]){sizeof(u32)}, (char *[]){(char *)"key"});
  
  return instance;
}

void u32_table_insert(u32_table * table, u32 * key, size_t count){
  void * array[] = {(void* )key};
  icy_table_inserts((icy_table *) table, array, count);
}

void u32_table_set(u32_table * table, u32 key){
  void * array[] = {(void* )&key};
  icy_table_inserts((icy_table *) table, array, 1);
}

void u32_table_lookup(u32_table * table, u32 * keys, size_t * out_indexes, size_t count){
  icy_table_finds((icy_table *) table, keys, out_indexes, count);
}

void u32_table_remove(u32_table * table, u32 * keys, size_t key_count){
...
void u32_table_clear(u32_table * table){
  icy_table_clear((icy_table *) table);
}

void u32_table_unset(u32_table * table, u32 key){
  u32_table_remove(table, &key, 1);
}

bool u32_table_try_get(u32_table * table, u32 * key){
  void * array[] = {(void* )key};
...
void u32_table_print(u32_table * table){
  icy_table_print((icy_table *) table);
}

size_t u32_table_iter(u32_table * table, u32 * keys, size_t keycnt, u32 * optional_keys_out, size_t * indexes, size_t cnt, size_t * iterator){
